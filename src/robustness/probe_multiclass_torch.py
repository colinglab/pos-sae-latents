...
def make_classifier(backend, C, max_iter, tol, device, torch_verbose):
    ...
    return LogisticRegression(
        ...
        solver="saga",  # saga's default multi_class="auto" already resolves to
                         # multinomial for >2 classes, so no need to set it explicitly
                         # (and newer sklearn removed the multi_class kwarg entirely)
        C=C,
...
def probe_column_multiclass(
    ...
    # Manual StratifiedKFold CV loop: TorchL1LogisticRegression doesn't
    # implement the sklearn estimator API (get_params/clone), so it can't be
    # passed to cross_val_score directly. Refitting per fold here is cheap
    # since each torch fit is fast.
    print(f"  Running {cv}-fold CV (backend={backend})...")
    skf = StratifiedKFold(n_splits=cv, shuffle=True, random_state=42)
    cv_scores = []
    for fold_i, (train_idx, val_idx) in enumerate(skf.split(X_sc, y)):
        ...
        fold_f1 = f1_score(y[val_idx], y_val_pred, average="macro")
        cv_scores.append(fold_f1)
        print(f"    fold {fold_i}: macro-F1 = {fold_f1:.4f}")
    cv_scores = np.array(cv_scores)

    clf = make_classifier(backend, C, max_iter, tol, device, torch_verbose)
    print(f"  Fitting final model on full data ({X_sc.shape[1]} features)...")
    t0 = time.time()
    clf.fit(X_sc, y)
...
def main():
    parser = argparse.ArgumentParser(
        description="Multiclass probe of SAE latents for a UD column, trained with "
                    "TorchL1LogisticRegression (full-batch FISTA in PyTorch) instead of "
                    "sklearn's saga solver. Same feature selection / CV / reporting as "
                    "probing/probe_multiclass_cv.py, just a faster solver."
    )
    ...
    parser.add_argument(
        ...
        help="Path to a pickle mapping POS -> list of important 'act_*' latent names "
             "(e.g. important_act_per_pos_layer30.pkl). Only latents in this set are "
             "considered as candidate features. Passed explicitly (rather than the "
             "hardcoded layer_n lookup in probing/probe_multiclass_cv.py) so a sweep "
             "script can vary it.",
    )
    ...
    base_name = f"{input_basename}_{args.target_col}_{iap_basename}_C{c_str}_multiclass_torch"
    ...
    metrics = {
        ...
        "cv_f1_macro_mean":    results["cv_f1_macro"],
        "cv_f1_macro_std":     results["cv_f1_macro_std"],
        ...
        "fit_seconds":         results["fit_seconds"],
